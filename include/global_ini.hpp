...
namespace IniHelper {
	...
	inline std::map<std::string, std::string>& get_ini() {
		//check this: http://stackoverflow.com/a/189162/2303236
		static std::map<std::string, std::string> ini;
		return ini;
	}

	//load ini from environment variable
	inline void iniLoad(const char** envp, const char *prefix="", const bool doOverWrite=false) {
	...
	template<>
	inline std::string iniGet(const std::string &key, const std::string &default_value) {
		static std::map<std::string, std::string>& ini = get_ini();
		std::map<std::string, std::string>::const_iterator itr=ini.find(key);
		if(itr!=ini.end()) {
			return itr->second;
	...
	//handle false/true
	template<>
	inline bool iniGet(const std::string &key, const bool &default_value) {
		static std::map<std::string, std::string>& ini = get_ini();
		std::map<std::string, std::string>::const_iterator itr = ini.find(key);
		if (itr != ini.end()) {
